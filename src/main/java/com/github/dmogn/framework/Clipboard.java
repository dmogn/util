...
public class Clipboard {
    ...
    static public void copyObject(BaseObject object) {
        String xmlText = com.github.dmogn.framework.DocumentSerializerToolkit.xmlSaveObject(object, false);
    ...
    public static org.jdom.Element fromXMLText(String xmlText) {
        if (com.github.dmogn.util.StringToolkit.findFirstFromToIgnoreCase(xmlText, "<?xml version=\"", 0, 2) != 0) {
            return null;
        }
    ...
    static public boolean pasteObject(BaseObject toObject) {
        ...
        try {
            com.github.dmogn.framework.DocumentManager.getCurrentDocument().getRegistry().pushStack();

            BaseObject object = loadObject(pasteElement(), toObject);
            object.setParentObject(toObject);
            com.github.dmogn.framework.DocumentManager.getCurrentDocument().getRegistry().resolveDependenciesLocal();
            com.github.dmogn.framework.DocumentManager.getCurrentDocument().getRegistry().resetGUIDsForObjectTree(object.getGUID());
            ...
            com.github.dmogn.framework.DocumentManager.getCurrentDocument().getRegistry().popStack();

            if (!com.github.dmogn.framework.DocumentManager.getCurrentDocument().checkConsistence()) {
    ...
    static public boolean pasteObject(BaseObject object, BaseObject toObject) {
        com.github.dmogn.framework.Module module = com.github.dmogn.framework.DocumentManager.getCurrentDocument().getModuleForObject(object);
        if (module == null) {
            return false;
        }
      ...
      for (int i=0; i<constructors.length; i++)
      {
        if (constructors[i].getParameterTypes().length == 1)
        {
          ...
          break;
            ...
            com.github.dmogn.framework.Module module = com.github.dmogn.framework.DocumentManager.getCurrentDocument().getModuleForClass(className);
